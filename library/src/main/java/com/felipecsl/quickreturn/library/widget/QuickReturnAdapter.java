...
public class QuickReturnAdapter extends DataSetObserver implements ListAdapter {
  ...
  public QuickReturnAdapter(ListAdapter wrappedAdapter) {
    this(wrappedAdapter, 1);
  }

  public QuickReturnAdapter(ListAdapter wrappedAdapter, int numColumns) {
  ...
  @Override public View getView(int position, View convertView, @NonNull ViewGroup parent) {
    ...
    if (position < numColumns) {
      ...
      v.measure(View.MeasureSpec
                    .makeMeasureSpec(parent.getWidth() / numColumns, View.MeasureSpec.AT_MOST),
                heightMeasureSpec);
      finalHeight = v.getMeasuredHeight();
    }

    if (position + numColumns < itemsVerticalOffset.length) {
      itemsVerticalOffset[position + numColumns] =
          itemsVerticalOffset[position] + finalHeight + verticalSpacing;
    }
  ...
  public int getMaxVerticalOffset() {
    ...
    List<Integer> items = new ArrayList<>(itemsVerticalOffset.length);
    for (int aMItemOffsetY : itemsVerticalOffset) {
  ...
  @Override public void onChanged() {
    ...
    System.arraycopy(itemsVerticalOffset, 0, newArray, 0,
                     Math.min(itemsVerticalOffset.length, newArray.length));
